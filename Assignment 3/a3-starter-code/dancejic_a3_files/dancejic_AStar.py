...
Uniform Cost Search of a problem space.
 ...
 Usage:
 python dancejic_AStar.py FranceWithDXHeuristic
'''
...
# STEP 1a. compute f(s0)
  g[initial_state]=0.0
  #Compute f(s)
  f[initial_state]=h(initial_state)

# STEP 1b. Put the start state on a priority queue called OPEN
...
#         and if it is already on CLOSED, delete the new instance.
    ...
    for op in Problem.OPERATORS:
      if op.precond(S):
        ...
        #if the new state is a lower priority, remove from closed and add to open
        #else remove from L
        if (new_state in CLOSED):
          #print("Already have this state, in CLOSED. del ...")
          if (new_f < f[new_state]):
            CLOSED.remove(new_state)#del CLOSED[new_state]
          else:
            del new_state
          continue
